...
# Chỉ theo dõi PUMPUSDT
SYMBOLS = {
    "PUMP_USDT": {"binance_symbol": "PUMPUSDT", "candle_interval": "5m", "limit": 2}
}
...
def fetch_latest_candle(symbol_config):
    ...
    try:
        ...
        error_msg = f"Lỗi khi lấy dữ liệu PUMPUSDT: {str(error)}"
        print(f"🚨 {error_msg}")
        send_telegram_alert(f"```{error_msg}```", is_critical=True)
        return None

def analyze_candle(candle):
    """Phân tích nến PUMPUSDT"""
    try:
        ...
        has_lower_wick = lower_wick_percent >= 1  # Ngưỡng 1%
        
        upper_wick = high_price - max(open_price, close_price)
        upper_wick_percent = (upper_wick / max(open_price, close_price)) * 100
        has_upper_wick = upper_wick_percent >= 1  # Ngưỡng 1%
        ...
        error_msg = f"Lỗi phân tích nến PUMPUSDT: {str(error)}"
        print(f"🚨 {error_msg}")
        send_telegram_alert(f"```{error_msg}```", is_critical=True)
        return None
...
━━━━━━━━━━━━━━
📈 Giá Mở: {analysis['open']:.8f}
📉 Giá Đóng: {analysis['close']:.8f}
🔺 Giá Cao: {analysis['high']:.8f}
🔻 Giá Thấp: {analysis['low']:.8f}
━━━━━━━━━━━━━━
...
def main():
    print("🟢 Khởi động trình theo dõi PUMP/USDT")
    ...
    while True:
        try:
            ...
            if now_utc.minute % 5 == 0 and now_utc.second < 5:
                ...
                candle_data = fetch_latest_candle(SYMBOLS["PUMP_USDT"])
                ...
                if candle_data:
                    ...
                    if analysis_result:
                        ...
                        print(f"   Mở: {analysis_result['open']:.8f} | Đóng: {analysis_result['close']:.8f}")
                        print(f"   Cao: {analysis_result['high']:.8f} | Thấp: {analysis_result['low']:.8f}")
